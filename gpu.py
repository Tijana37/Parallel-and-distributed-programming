...
def cost_function(theta1, theta2, input_layer_size, hidden_layer_size, output_layer_size, inputs, labels, regular=0):
    ...
    for training_index in xrange(len(inputs)):
        ...
        for k in xrange(output_layer_size):
            cost += -outputs[k] * math.log(output_layer[training_index][k]) - (1 - outputs[k]) * math.log(
                1 - output_layer[training_index][k])
    cost /= len(inputs)
    time_end = time.time()
...
def gradient_descent(inputs, labels, learningrate=0.8, iteration=50):
    ...
    for i in xrange(iteration):
        ...
        time_end = time.time()
        print('Iteration {0} (learning rate {2}, iteration {3}), cost: {1}, time: {4}'.format(i + 1, cost, learningrate,
                                                                                              iteration,
                                                                                              time_end - time_start))
...
if __name__ == '__main__':
    ...
    cost, (theta1_grad, theta2_grad) = cost_function(theta1, theta2, Input_layer_size, Hidden_layer_size,
                                                     Output_layer_size, inputs, labels, regular=0)
    print('cost:', cost)
